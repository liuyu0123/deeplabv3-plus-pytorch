...
def parse_args():
    parser = argparse.ArgumentParser(description='DeepLabV3+ 水域分割训练（优化版）')
    ...
    # 优化器配置
    parser.add_argument('--lr', '--learning-rate', dest='lr', type=float, default=7e-3,
                        help='初始学习率（支持--lr或--learning-rate）')
    ...
    parser.add_argument('--model-name', type=str, default=None,
                        help='模型保存文件名（不含扩展名），例如 deeplab_exp1')
    parser.add_argument('--log-name', type=str, default=None,
                        help='日志文件名（不含扩展名），例如 deeplab_exp1')
