# predict_water_deeplab_pro.py
# DeepLabV3+ 版本：与 predict_water_best_pro.py 保持相同的命令行接口和评估功能
import os
import argparse
import numpy as np
import torch
...
# ==================== 主函数 ====================
def main():
    parser = argparse.ArgumentParser(description='DeepLabV3+ 水体分割推理工具（自适应修复版）')
    ...
    parser.add_argument('--downsample_factor', type=int, default=16, choices=[8, 16], help='下采样倍数')
    ...
    for img_path in input_paths:
        ...
        try:
            ...
            # 向下对齐到32的倍数（DeepLabV3+ 通常需要 32 的倍数）
            nw = max(32, (nw // 32) * 32)
            nh = max(32, (nh // 32) * 32)
            
            input_shape = (nh, nw)  # (height, width)
